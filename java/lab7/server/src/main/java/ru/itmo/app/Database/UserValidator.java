...
public class UserValidator implements IUserValidator {
    ...
    private void validateUsernameLength(String username) throws AuthException {
        if (username.length() > MAX_USERNAME_LENGTH) {
            throw new AuthException("Username length cannot exceed " + MAX_USERNAME_LENGTH + " characters.");
        }
        if (username.length() < MIN_USERNAME_LENGTH) {
            throw new AuthException("Username length cannot be less than " + MIN_USERNAME_LENGTH + " characters.");
        }
    }
    ...
    private void validatePasswordLength(String password) throws AuthException {
        if (password.length() > MAX_PASSWORD_LENGTH) {
            throw new AuthException("Password length cannot exceed " + MAX_PASSWORD_LENGTH + " characters.");
        }
        if (password.length() < MIN_PASSWORD_LENGTH) {
            throw new AuthException("Password length cannot be less than " + MIN_PASSWORD_LENGTH + " characters.");
        }
    }
    private void validateDifficulty(String password) throws AuthException {
        if (!password.matches(".*[a-zA-Z].*")) {
            throw new AuthException("Password must contain at least one latin letter.");
        }
