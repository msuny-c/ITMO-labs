...
public class HumanGetter {
    ...
    public HumanGetter(String[] args, ScannerManager scannerManager) {
        this.args = args;
    ...
    private void setCoordinates(HumanBeing object)  {
        while (true) {
            try {
                ...
                System.out.println("Incorrect arguments. Must be: [x] [y]");
    ...
    private void setCar(HumanBeing object) {
        while (true) {
            try {
                System.out.println("Enter Car object. Syntax: [name] [cool]");
                String inputLine = scannerManager.nextLine();
