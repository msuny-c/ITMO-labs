...
public class UpdateCommand extends ServerCommand {
    ...
    public void execute(String[] args, ScannerManager scannerManager) {
        try {
            ...
            CommandRequest request = new CommandRequest(getName(), newHuman, Arrays.copyOfRange(args, 0, 1));
            send(request);
        } catch (IllegalArgumentException exception) {
            System.out.println("Incorrect arguments. Should be: update {id} {name} {realHero} {hasToothPick} {impactSpeed}");
        }
    }
